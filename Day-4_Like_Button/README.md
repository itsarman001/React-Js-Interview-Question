...
Click the button to send a like/unlike request.

The button state and label will update based on the API response:

Success: Toggles the liked state.

Failure: Displays an error message.

While the API call is in progress, a spinner icon replaces the heart icon, and the button is disabled.
